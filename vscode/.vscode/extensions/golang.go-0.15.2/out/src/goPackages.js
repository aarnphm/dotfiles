...
exports.getNonVendorPackages = exports.getImportablePackages = exports.getAllPackages = void 0;
...
function gopkgs(workDir) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const cmd = cp.spawn(gopkgsBinPath, args, { env: goEnv_1.toolExecutionEnvironment() });
        const chunks = [];
        const errchunks = [];
        let err;
        ...
        cmd.on('close', () => {
            ...
            const goroot = goPath_1.getCurrentGoRoot();
...
function getNonVendorPackages(currentFolderPath) {
    ...
    if (!goRuntimePath) {
        console.warn(`Failed to run "go list" to find packages as the "go" binary cannot be found in either GOROOT(${goPath_1.getCurrentGoRoot()}) PATH(${goPath_1.envPath})`);
        return;
    }
    return new Promise((resolve, reject) => {
        const childProcess = cp.spawn(goRuntimePath, ['list', '-f', 'ImportPath: {{.ImportPath}} FolderPath: {{.Dir}}', './...'], { cwd: currentFolderPath, env: goEnv_1.toolExecutionEnvironment() });
        const chunks = [];
