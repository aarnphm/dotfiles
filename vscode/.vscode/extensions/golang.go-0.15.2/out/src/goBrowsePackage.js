...
function showPackageFiles(pkg, showAllPkgsIfPkgNotFound, workDir) {
    ...
    if (!goRuntimePath) {
        return vscode.window.showErrorMessage(`Failed to run "go list" to fetch packages as the "go" binary cannot be found in either GOROOT(${goPath_1.getCurrentGoRoot()}) or PATH(${goPath_1.envPath})`);
    }
