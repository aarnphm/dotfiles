    ...
    , handleEventHook = ewmhDesktopsEventHook <+> fullscreenEventHook <+> focusFollow <+>
                    (\e -> case e of
                        PropertyEvent{ ev_window = w } -> do
                            isAlacritty <- runQuery (className =? "Alacritty") w
                            if not isAlacritty then hintsEventHook e else return (All True)
                        _ -> return (All True))
    , workspaces = myTopics
    , manageHook = mconcat
                    [manageSpawn
...
myXPConfig :: XPConfig
myXPConfig = greenXPConfig { font = "xft:Inconsolata:pixelsize=12:autohint=true" }
...
myKeys c =
    ...
    ,("M-S-d", restart "alacritty -e xmonad" False)
...
myTopicConfig = TopicConfig
  ...
  , topicActions = M.fromList $
      [ ("xm-conf", spawnShellIn ".xmonad/lib/XMonad/Layout" >>
                        spawn "alacritty -e vim ~/.xmonad/xmonad.hs")
       , ("xmc"    , spawnShell >*> 2)
       , ("xmobar" , spawnShellIn "wip/x11-wm/xmobar/Plugins" >*> 2)
       , ("music"  , spawn "alacritty -e ncmpc -h /home/aavogt/.mpd/socket" >> spawn "export MPD_HOST=192.168.1.2; mpc && alacritty -e ncmpc -h 192.168.1.2")
       , ("mail"   , spawnOn "mail" "alacritty -e mutt")
       , ("irc"    , spawnOn "irc" "alacritty --title irc -e ssh engage")
