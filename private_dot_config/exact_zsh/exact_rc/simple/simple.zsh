...
prompt_simple_prompt_prerender() {
  ...
  [[ -n $prompt_simple_cmd_exec_time ]] && rprompt_parts+=('${prompt_simple_state[accent]}%F{$prompt_simple_colors[execution_time]}${prompt_simple_cmd_exec_time}%f')

  # Contruct new prompt after cleaning
  local -ah ps1
  local -ah rps1
  ps1=(${(j..)prompt_parts}$prompt_indicator)
