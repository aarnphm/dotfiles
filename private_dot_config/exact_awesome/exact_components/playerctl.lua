--
-- Provides:
-- components::playerctl::status
--      playing (boolean)
-- components::playerctl::title_artist_album
--      title (string)
--      artist    (string)
--      album_path (string)
-- components::playerctl::position
--      interval_sec (number)
--      length_sec (number)
-- components::playerctl::player_stopped
--
    ...
    awful.spawn.easy_async({
        ...
        awful.spawn.with_line_callback(status_cmd, {
            stdout = function(line)
                ...
                awesome.emit_signal("components::playerctl::status", playing)
            end
        })
    end)
end
        ...
        awful.spawn.easy_async_with_shell(length_cmd, function(length)
            ...
            if length_sec and interval_sec then
                if interval_sec >= 0 and length_sec > 0 then
                    awesome.emit_signal("components::playerctl::position",
            ...
            stdout = function(line)
                ...
                awful.spawn.easy_async_with_shell(art_script, function(out)
                    ...
                    if title and title ~= "" then
                        awesome.emit_signal(
                            "components::playerctl::title_artist_album", title,
                            artist, album_path)
                    else
                        awesome.emit_signal("components::playerctl::player_stopped")
                    end
                end)
            end
        })
